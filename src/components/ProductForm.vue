...
const rules = reactive<FormRules<TProduct>>({
  ...
  newPrice: [
    {
      required: true,
      message: "Giá mới không được bỏ trống",
      trigger: ["change", "blur"],
    },
  ],
  ...
  oldPrice: [
    {
      required: true,
      message: "Giá cũ không được bỏ trống",
      trigger: ["change", "blur"],
    },
  ],
  ...
  await ruleFormRef?.value?.validate(async (valid, fields) => {
    if (valid) {
      actionType.value === ACTION_ENUM.CREATE
        ? await createProduct({ ...ruleForm, image: ruleForm.details[0] })
  ...
  <div class="product-form-container">
    <el-card class="main">
      ...
      <el-form ref="ruleFormRef" :rules="rules" :model="ruleForm">
        ...
        <el-button @click="handleAddImage" style="position: absolute; right: 0"
          ><el-icon><Plus /></el-icon
        ></el-button>
        <el-form-item
          v-for="(_item, index) in imageSize"
          label="Ảnh sản phẩm"
          label-position="top"
        >
          <BaseUpload
            @change="handleChangeFile"
            :url="ruleForm.details[index]"
          />
        </el-form-item>
      </el-form>
    </el-card>
