...
export const useUser = () => {
  ...
  const getOtp = async (email: string) => {
    try {
      await request.post("/auth/otp", { email });
  ...
  const confirmOtp = async (otp: number) => {
    try {
      await request.post("/auth/confirm-otp", { otp });
  ...
  const checkEmailExisted = async (email: string) => {
    try {
      await request.post("/auth/check-email-existed", { email });
